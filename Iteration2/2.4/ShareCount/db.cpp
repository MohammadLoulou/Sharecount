...
void db::creerCagnotte(std::string nom){
    if(!db::dataBase.isOpen())
    {
        db::dataBase = ConnexionDB();
    }
    ...
    query.prepare("INSERT INTO cagnotte(nom,argent) VALUES(:nom,:argent);");
    query.bindValue(":mail",QString::fromStdString(nom));
    query.bindValue(":argent",0);
    query.exec();
}

void db::supprCagnotte(std::string nom){
    if(!db::dataBase.isOpen())
    {
        db::dataBase = ConnexionDB();
    }
    QSqlQuery query(db::dataBase);
    query.prepare("DELETE FROM cagnotte WHERE nom = :nom");
